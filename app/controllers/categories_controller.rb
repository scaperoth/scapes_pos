class CategoriesController < ApplicationController
    ...
    def set_product_category(category)
        set_product_category_by_search_term category

        set_product_category_by_skus category
      end

    def set_product_category_by_search_term(category)
        unless category.search_term.empty?
            ...
            by_search_term = Product.where(team: current_team).where(condition_string, condition_vars)
            add_product_category by_search_term, category
        end
    end

    def set_product_category_by_skus(category)
        unless category.products_list.empty?
            skus = category.products_list.split(/[\s,'\t]/)
            by_product_skus = Product.where(team: current_team).where(sku: skus)
            add_product_category by_product_skus, category
        end
    end
    ...
    def category_params
        form_params = params.require(:category).permit(:name, :price, :search_term, :products_list)
